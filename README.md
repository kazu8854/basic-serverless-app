...
## 開発・拡張のルール (Architecture Rails)
### 1. 新しい機能（ドメイン）の追加方法
機能を追加する際はサイロ化を防ぐため、以下の明確な構造ルール（Rails）に従ってください。

*   **フロントエンドのルーティングとAPI呼び出し:**
    新ページは `packages/frontend/src/pages/` に作成し、`App.tsx` のReact Routerに追加します。
    バックエンドとの通信は `fetch` を直接書かず、**必ず `src/api/client.ts` が提供する Hono RPC クライアント (`client.api.x`) を使用**します。これによりURLミスや型の不一致がコンパイル時にすべて検出されます。
*   **バックエンドのルート分割:**
    新しいAPI群は `packages/backend/src/api/<domain>.ts` のように別ファイルに切り出し、新しい Hono インスタンスで定義します（例: `userApp`）。
    その後、`src/index.ts` にて `app.route('/api/<domain>', domainApp)` としてマウントしてください。これにより Hono RPC のための深い型構造がフロントエンドに安全にエクスポートされます。

### 2. テストの作成と運用 (Quality Assurance)
本プロジェクトでは品質の指標として継続的なテストとカバレッジ計測を採用しています。
ワークスペースごとに最適なテストフレームワークが分かれています：
*   **Infrastructure (CDK):** 既存の `Jest` を使用 (`npm run test -w packages/infrastructure`)
*   **Backend & Frontend:** 高速な `Vitest` を使用。
    *   **Backend:** Hono の `app.request()` を用いたエンドツーエンド（In-memory）の結合テストを `packages/backend/test/` に配置します。
    *   **Frontend:** `react-testing-library` を用いたコンポーネントテストを `packages/frontend/test/` に配置します。

**カバレッジの計測:**
各ワークスペースで `npm run test:coverage` を実行すると、`coverage/` ディレクトリ配下に詳細なHTMLおよびLCOVレポートが出力されます。CI等で品質指標として活用してください。

## Getting Started（ローカル開発の使い始め方）
